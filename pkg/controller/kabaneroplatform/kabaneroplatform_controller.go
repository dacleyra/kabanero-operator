...
func (r *ReconcileKabanero) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	processDeletion(ctx, instance, r.client, reqLogger)
...
func processDeletion(ctx context.Context, k *kabanerov1alpha1.Kabanero, client client.Client, reqLogger logr.Logger) error {
	...
	if k.ObjectMeta.DeletionTimestamp.IsZero() {
		if !foundFinalizer {
